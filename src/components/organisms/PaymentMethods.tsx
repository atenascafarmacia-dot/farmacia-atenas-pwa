...
interface PaymentMethodsProps {
  /** The order's payment currency; accounts tagged for other currencies are hidden. */
  currency: Currency;
}
 ...
 */
export function PaymentMethods({ currency }: PaymentMethodsProps) {
  const methods = payment.methods.filter(
    (method) => !method.currencies || method.currencies.includes(currency),
  );
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-soft">
      ...
      <ul className="mt-3 flex flex-col gap-3" role="list">
        {methods.map((method) => (
          <li key={method.name} className="rounded-xl border border-border bg-surface p-3">
            ...
            <dl className="mt-2 flex flex-col gap-1 text-sm">
              {method.rows.map((row) => (
                <div key={row.label} className="flex justify-between gap-3">
                  ...
                  <dd className="select-all text-right font-medium text-ink">{row.value}</dd>
                </div>
              ))}
            </dl>
